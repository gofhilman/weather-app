...
function updateMenu() {
  ...
  for (let pinNumber = 0; pinNumber < page.locations.length; pinNumber++) {
    ...
    import(
      `./assets/weather-icon/${page.locations[pinNumber].getCurrentWeather().icon}.svg`
    )
      .then((source) => (weatherIcon.src = source.default))
      .catch((error) => console.error("Failed to load icon:", error));
    weatherDesc.textContent =
      page.locations[pinNumber].getCurrentWeather().conditions;
    let temperatureUnit = page.unit === "metric" ? "C" : "F";
    placeTemp.textContent =
      `${Math.round(page.locations[pinNumber].getCurrentWeather().temp)} ` +
      `\u00B0${temperatureUnit}`;
